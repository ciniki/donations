...
function ciniki_donations_donationUpdate(&$ciniki) {
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'date_received'=>array('required'=>'no', 'blank'=>'no', 'type'=>'date', 'name'=>'Date Received'), 
        'amount'=>array('required'=>'no', 'blank'=>'no', 'type'=>'currency', 'name'=>'Amount'), 
        'date_issued'=>array('required'=>'no', 'blank'=>'yes', 'type'=>'date', 'name'=>'Date Issued'), 
    ...
    $rc = ciniki_donations_checkAccess($ciniki, $args['tnid'], 'ciniki.donations.donationUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    //
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.donations.donation', $args['donation_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    $rc = ciniki_donations_donationLoad($ciniki, $args['tnid'], $args['donation_id']);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
