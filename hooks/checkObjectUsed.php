...
// -------
//
function ciniki_donations_hooks_checkObjectUsed($ciniki, $business_id, $args) {
    ...
    if( $args['object'] == 'ciniki.customers.customer' ) {
        ...
        $strsql = "SELECT 'items', COUNT(*) "
            ...
            . "";
        $rc = ciniki_core_dbCount($ciniki, $strsql, 'ciniki.donations', 'num');
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
        if( isset($rc['num']['items']) && $rc['num']['items'] > 0 ) {
            $used = 'yes';
            $count = $rc['num']['items'];
            $msg .= ($msg!=''?' ':'') . "There " . ($count==1?'is':'are') . " $count donation" . ($count==1?'':'s') . " for this customer.";
        }
    }
